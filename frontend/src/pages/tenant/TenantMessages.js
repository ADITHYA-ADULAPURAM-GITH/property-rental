  ...
  return (
    <div className="animate-fade-in">
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Communicate with your landlord.</p>
      </div>
        ...
        <div style={{ width: '320px', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {loading ? (
              ...
              <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.875rem' }}>No conversations yet.</div>
            ) : (
              filtered.map(contact => (
                <div
                  ...
                  style={{
                    ...
                    background: (activeChat?.user?._id || activeChat?.user) === (contact.user?._id || contact.user)
                      ? 'rgba(59, 130, 246, 0.1)' : 'transparent',
                    transition: 'background 0.2s', display: 'flex', gap: '1rem'
                  }}
                >
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', flexShrink: 0 }}>
          ...
          {!activeChat ? (
            ...
            <>
              <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>
              ...
              <div style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {threadLoading ? (
                  ...
                  <div style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.875rem' }}>No messages yet. Start the conversation!</div>
                ) : (
                  thread.map((msg, i) => {
                    const isMe = msg.sender?._id !== (activeChat.user?._id || activeChat.user);
                    return (
